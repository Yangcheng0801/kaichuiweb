...
const PLAN_STATUS_LABELS: Record<string, { label: string; cls: string }> = {
  active:   { label: '在售', cls: 'bg-emerald-100 text-emerald-700' },
  inactive: { label: '已下架', cls: 'bg-gray-100 text-gray-500' },
}
            ...
            return (
              <button key={t.key} onClick={() => setActiveTab(t.key)}
                className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium rounded-t-lg transition
                  ${isActive ? 'bg-amber-50 text-amber-700 border border-b-0 border-amber-200' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          ...
          <button onClick={handleSeed}
            className="flex items-center gap-1.5 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 text-sm">
            <Zap className="w-4 h-4" /> 初始化默认套餐
          </button>
        </div>
        <button onClick={loadPlans} className="p-2 text-gray-400 hover:text-gray-600">
        ...
        {plans.map(plan => (
          <div key={plan._id} className="bg-white rounded-xl shadow-sm border p-5 hover:shadow-md transition group">
            <div className="flex items-start justify-between mb-3">
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${PLAN_STATUS_LABELS[plan.status]?.cls || 'bg-gray-100 text-gray-500'}`}>
                {PLAN_STATUS_LABELS[plan.status]?.label || plan.status}
              </span>
            </div>

            <p className="text-sm text-gray-500 mb-3 line-clamp-2">{plan.description || '暂无描述'}</p>
            ...
            <div className="flex flex-wrap gap-1.5 mb-3 text-xs">
              ...
              {plan.benefits?.freeRounds > 0 && <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded">免费{plan.benefits.freeRounds}轮</span>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-auto p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label className="text-sm font-medium text-gray-700">套餐编码 *</label>
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">有效月数 (0=用完即止)</label>
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">押金 (¥)</label>
            ...
            {/* 权益配置 */}
            <h3 className="text-sm font-bold text-gray-700 mb-2 mt-4 flex items-center gap-1"><Gift className="w-4 h-4" /> 权益配置</h3>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div>
                <label className="text-xs text-gray-500">免费轮次 (0=不限)</label>
              ...
              <div>
                <label className="text-xs text-gray-500">续打折扣 (0.7=7折)</label>
              ...
              <div>
                <label className="text-xs text-gray-500">带客名额</label>
              ...
              <div>
                <label className="text-xs text-gray-500">带客折扣</label>
              ...
              <div>
                <label className="text-xs text-gray-500">最大绑定人数</label>
              ...
              <div>
                <label className="text-xs text-gray-500">关联身份编码</label>
            ...
            {/* 积分规则 */}
            <h3 className="text-sm font-bold text-gray-700 mb-2 flex items-center gap-1"><Star className="w-4 h-4" /> 积分规则</h3>
            <div className="grid grid-cols-3 gap-3 mb-6">
              <div>
                <label className="text-xs text-gray-500">赚取倍率</label>
              ...
              <div>
                <label className="text-xs text-gray-500">开卡赠送积分</label>
              ...
              <div>
                <label className="text-xs text-gray-500">生日积分倍数</label>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: '有效会籍', value: stats.active, cls: 'text-emerald-600', bg: 'bg-emerald-50' },
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {filtered.map(m => {
              const st = STATUS_LABELS[m.status] || { label: m.status, cls: 'bg-gray-100 text-gray-500' }
              const freeRounds = m.benefits?.freeRounds || 0
              const roundsUsed = m.usage?.roundsUsed || 0
              return (
                <tr key={m._id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono text-xs text-gray-500">{m.membershipNo}</td>
                  <td className="px-4 py-3">
                    <div className="font-medium text-gray-800">{m.playerName}</div>
                    <div className="text-xs text-gray-400">{m.phoneNumber}</div>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-gray-700">{m.planName}</span>
                    <span className="text-xs text-gray-400 ml-1">({CATEGORY_LABELS[m.planCategory] || m.planCategory})</span>
                  </td>
                    ...
                    {freeRounds > 0 ? (
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-gray-100 rounded-full h-1.5 max-w-[80px]">
                          <div className="bg-emerald-500 h-1.5 rounded-full" style={{ width: `${Math.min(100, (roundsUsed / freeRounds) * 100)}%` }} />
                        </div>
                        <span className="text-gray-500">{roundsUsed}/{freeRounds}轮</span>
                      </div>
                    ...
                    <div className="flex items-center justify-end gap-1">
                      {['active', 'expiring', 'expired'].includes(m.status) && (
                        <button onClick={() => { setSelectedMembership(m); setRenewForm({ months: 12, amount: 0, method: 'card' }); setShowRenew(true) }}
                          className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded" title="续费">
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={7} className="text-center py-12 text-gray-400">暂无会籍记录</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowCreate(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
            ...
            {/* 球员搜索 */}
            <div className="mb-4">
              <label className="text-sm font-medium text-gray-700 mb-1 block">选择球员</label>
              <div className="flex gap-2">
                ...
                <button onClick={handleSearchPlayer} className="px-3 py-2 bg-gray-100 rounded-lg hover:bg-gray-200">
            ...
            {/* 选择套餐 */}
            <div className="mb-4">
              <label className="text-sm font-medium text-gray-700 mb-1 block">选择套餐</label>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div>
                <label className="text-sm font-medium text-gray-700">支付金额</label>
      ...
      {showRenew && selectedMembership && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowRenew(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div>
                <label className="text-sm font-medium text-gray-700">续费月数</label>
              ...
              <div>
                <label className="text-sm font-medium text-gray-700">续费金额</label>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={handleRenew} className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700">确认续费</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      ...
      {stats && (
        <div className="grid grid-cols-4 gap-4">
          {[
            { label: '累计发放', value: stats.totalEarned?.toLocaleString() || '0', icon: ArrowUpRight, cls: 'text-emerald-600', bg: 'bg-emerald-50' },
            { label: '累计消费', value: stats.totalRedeemed?.toLocaleString() || '0', icon: ArrowDownRight, cls: 'text-red-500', bg: 'bg-red-50' },
            { label: '累计过期', value: stats.totalExpired?.toLocaleString() || '0', icon: Clock, cls: 'text-gray-500', bg: 'bg-gray-50' },
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={loadData} className="ml-auto p-2 text-gray-400 hover:text-gray-600">
          <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
        </button>
      </div>
          ...
          <tbody className="divide-y">
            {filtered.map((t, i) => (
              <tr key={t._id || i} className="hover:bg-gray-50">
                <td className="px-4 py-3 text-xs text-gray-400">{t.createdAt?.slice(0, 16).replace('T', ' ')}</td>
                <td className="px-4 py-3 text-gray-700">{t.playerName}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${t.amount > 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                    {POINT_TYPE_LABELS[t.type] || t.type}
                  </span>
                </td>
                <td className={`px-4 py-3 text-right font-medium ${t.amount > 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                  {t.amount > 0 ? '+' : ''}{t.amount}
                </td>
                <td className="px-4 py-3 text-right text-gray-500">{t.balanceAfter}</td>
                <td className="px-4 py-3 text-xs text-gray-400 max-w-[200px] truncate">{t.description}</td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr><td colSpan={6} className="text-center py-12 text-gray-400">暂无积分记录</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {showAdjust && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowAdjust(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-4">
              <label className="text-sm font-medium text-gray-700">调整数额 (正数=增加，负数=扣减)</label>
    ...
    <div className="space-y-6">
      ...
      {overview && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: '活跃会籍', value: overview.active || 0, icon: Users, cls: 'text-emerald-600', bg: 'bg-emerald-50' },
      ...
      {overview?.byStatus && (
        <div className="bg-white rounded-xl shadow-sm border p-5">
          <h3 className="text-sm font-bold text-gray-700 mb-4">会籍状态分布</h3>
      ...
      {plansSummary && (
        <div className="bg-white rounded-xl shadow-sm border p-5">
          <h3 className="text-sm font-bold text-gray-700 mb-4">套餐销售概况</h3>
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div className="bg-gray-50 rounded-lg p-3 text-center">
              <p className="text-xs text-gray-400">在售套餐数</p>
              <p className="text-xl font-bold text-gray-700">{plansSummary.activePlans || 0}</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3 text-center">
              <p className="text-xs text-gray-400">已售会籍数</p>
              <p className="text-xl font-bold text-gray-700">{plansSummary.totalMemberships || 0}</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3 text-center">
          ...
          {plansSummary.byCategory && Object.keys(plansSummary.byCategory).length > 0 && (
            <div>
              <h4 className="text-xs font-medium text-gray-500 mb-2">按类型</h4>
              <div className="space-y-1.5">
                {Object.entries(plansSummary.byCategory).map(([cat, data]: any) => (
                  <div key={cat} className="flex items-center justify-between text-sm bg-gray-50 rounded px-3 py-2">
                    <span className="text-gray-600">{CATEGORY_LABELS[cat] || cat}</span>
                    <span className="text-gray-400">{data.count} 人 · ¥{(data.revenue || 0).toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
