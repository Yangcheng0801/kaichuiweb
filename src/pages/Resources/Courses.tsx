...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  active:      { label: '开放中',  color: 'bg-success/10 text-success' },
  maintenance: { label: '维护中',  color: 'bg-yellow-100  text-yellow-700'  },
  closed:      { label: '已关闭',  color: 'bg-secondary    text-muted-foreground'    },
}
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground text-sm">暂无球场，点击右上角新增</div>
      ) : (
          ...
          {list.map(item => (
            <div key={item._id} className="flex items-center justify-between bg-secondary/50 rounded-xl px-5 py-4 hover:bg-secondary transition-colors">
              <div className="flex items-center gap-4 min-w-0">
                <div className="w-10 h-10 rounded-xl bg-success/10 text-success flex items-center justify-center font-bold text-sm flex-shrink-0">
                  {item.holes}H
                </div>
              ...
              <div className="flex items-center gap-3 flex-shrink-0 ml-4">
                ...
                <button onClick={() => openEdit(item)} className="p-1.5 text-muted-foreground hover:text-success transition-colors">
                  <Pencil size={14} />
                </button>
                <button onClick={() => setDeleteId(item._id!)} className="p-1.5 text-muted-foreground hover:text-red-500 transition-colors">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">球场名称 <span className="text-red-500">*</span></label>
                <input
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <h3 className="font-semibold text-foreground mb-2">确认删除球场？</h3>
