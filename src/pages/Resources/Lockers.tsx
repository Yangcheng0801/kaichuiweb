...
const STATUS_COLORS: Record<string, { bg: string; border: string; text: string; label: string }> = {
  available:   { bg: 'bg-success/10',  border: 'border-success/30', text: 'text-success', label: '可用' },
  occupied:    { bg: 'bg-red-50',      border: 'border-red-300',     text: 'text-red-600',     label: '占用' },
  maintenance: { bg: 'bg-secondary',    border: 'border-border',    text: 'text-muted-foreground',    label: '维护' },
  retired:     { bg: 'bg-secondary/50',     border: 'border-border',    text: 'text-muted-foreground',    label: '停用' },
}
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            ...
            { label: '可用', value: stats.available, color: 'text-success' },
      ...
      <div className="flex bg-card rounded-xl p-1 shadow-sm border border-border w-fit">
        {[
          ...
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-2 text-sm rounded-lg font-medium transition-all ${tab === t.key ? 'bg-success/10 text-success shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="ml-auto flex items-center gap-2 text-[11px] text-muted-foreground">
              <span className="inline-block w-3 h-3 bg-success/30 rounded-sm border border-success" /> 日租
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-muted-foreground"><p className="text-sm">暂无更衣柜</p></div>
          ) : (
            Object.entries(grouped).map(([area, items]) => (
              <div key={area}>
                <h4 className="text-sm font-medium text-muted-foreground mb-2">{area} <span className="text-muted-foreground">({items.length})</span></h4>
                    ...
                    return (
                      <div key={l._id} className="group relative"
                        ...
                        <div className={`${sc.bg} ${isLongTerm ? 'border-amber-400' : sc.border} border-2 rounded-lg p-2 text-center cursor-pointer transition-all hover:shadow-md`}>
                          ...
                          {l.currentPlayerName && <div className="text-[10px] text-muted-foreground mt-0.5 truncate">{l.currentPlayerName}</div>}
          ...
          <div className="bg-card rounded-2xl shadow-sm border border-border overflow-hidden">
            {contracts.length === 0 ? (
              ...
              <div className="divide-y divide-border/50">
                {contracts.map(c => {
                  const st = CONTRACT_STATUS[c.status] || { label: c.status, cls: 'bg-secondary text-muted-foreground' }
                  return (
                    <div key={c._id} className="px-5 py-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-foreground">{c.lockerNo}</span>
                          ...
                          <span className="text-[11px] text-muted-foreground">{RENTAL_TYPE_MAP[c.rentalType] || c.rentalType}</span>
                        </div>
            ...
            <div className="divide-y divide-border/50">
              {lockers.filter(l => l.keyInfo?.keyNo).map(l => (
                <div key={l._id} className="px-5 py-3 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-foreground">{l.lockerNo} - {l.keyInfo!.keyNo}</div>
                  ...
                  {!l.keyInfo!.returnedAt ? (
                    ...
                    <span className="text-[11px] text-success bg-success/10 px-2 py-0.5 rounded-full">已归还</span>
                  )}
                </div>
              ))}
              {lockers.filter(l => l.keyInfo?.keyNo).length === 0 && (
                <div className="p-12 text-center text-muted-foreground text-sm">暂无钥匙发放记录</div>
              )}
            </div>
          </div>
        </>
      ...
      {tab === 'logs' && (
        <>
          <div className="flex items-center gap-3">
            <h3 className="text-sm font-semibold text-foreground">使用记录</h3>
            {selectedLocker && (
              <span className="text-sm text-success bg-success/10 px-3 py-1 rounded-full">
              ...
              <div className="divide-y divide-border/50">
                {usageLogs.map(log => (
                  <div key={log._id} className="px-5 py-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-foreground">{ACTION_MAP[log.action] || log.action}</div>
                      <div className="text-xs text-muted-foreground">{log.note}</div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      {log.playerName && <div className="text-xs text-muted-foreground">{log.playerName}</div>}
          ...
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="flex gap-2">
                <button onClick={() => setFormMode('single')} className={`flex-1 py-2 text-sm rounded-lg border transition-all ${formMode === 'single' ? 'bg-success/10 border-success text-success font-medium' : 'border-border text-muted-foreground'}`}>单个创建</button>
                <button onClick={() => setFormMode('batch')} className={`flex-1 py-2 text-sm rounded-lg border transition-all ${formMode === 'batch' ? 'bg-success/10 border-success text-success font-medium' : 'border-border text-muted-foreground'}`}>批量创建</button>
              </div>
              {formMode === 'single' ? (
                <>
                  <div><label className="block text-xs font-medium text-muted-foreground mb-1">编号</label><input value={formLockerNo} onChange={e => setFormLockerNo(e.target.value)} placeholder="如：A-101" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                  <div><label className="block text-xs font-medium text-muted-foreground mb-1">区域</label><input value={formArea} onChange={e => setFormArea(e.target.value)} placeholder="如：A区" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                </>
              ) : (
                <>
                  <div><label className="block text-xs font-medium text-muted-foreground mb-1">编号前缀</label><input value={batchPrefix} onChange={e => setBatchPrefix(e.target.value)} placeholder="如：A-" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="block text-xs font-medium text-muted-foreground mb-1">起始</label><input type="number" value={batchStart} onChange={e => setBatchStart(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                    <div><label className="block text-xs font-medium text-muted-foreground mb-1">结束</label><input type="number" value={batchEnd} onChange={e => setBatchEnd(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                  </div>
                  <div><label className="block text-xs font-medium text-muted-foreground mb-1">区域</label><input value={batchArea} onChange={e => setBatchArea(e.target.value)} placeholder="如：A区" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                </>
              )}
              <div className="grid grid-cols-2 gap-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">租户姓名</label><input value={cfTenantName} onChange={e => setCfTenantName(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">手机号</label><input value={cfTenantPhone} onChange={e => setCfTenantPhone(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">租赁类型</label>
                <div className="flex gap-2">
                  {(['monthly', 'annual'] as const).map(t => (
                    <button key={t} onClick={() => setCfRentalType(t)} className={`flex-1 py-2 text-sm rounded-lg border transition-all ${cfRentalType === t ? 'bg-success/10 border-success text-success font-medium' : 'border-border text-muted-foreground'}`}>
                      {t === 'monthly' ? '月租' : '年租'}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">开始日期</label><input type="date" value={cfStartDate} onChange={e => setCfStartDate(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">结束日期</label><input type="date" value={cfEndDate} onChange={e => setCfEndDate(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
              </div>
              <div><label className="block text-xs font-medium text-muted-foreground mb-1">合同费用（元）</label><input type="number" value={cfFee} onChange={e => setCfFee(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
            </div>
            <div className="flex gap-3 px-6 py-4 border-t border-border">
      ...
      {showKeyDialog && selectedLocker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-sm">
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="text-sm text-muted-foreground">更衣柜：<span className="font-semibold text-foreground">{selectedLocker.lockerNo}</span></div>
              {keyAction === 'issue' ? (
                <>
                  <div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1">编号</label>
                    <input value={keyNo} onChange={e => setKeyNo(e.target.value)} placeholder="如：K-A101" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                  </div>
                </>
              ) : (
