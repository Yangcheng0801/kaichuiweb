...
const STATUS_MAP: Record<string, { label: string; bg: string; text: string }> = {
  available:     { label: '空净',   bg: 'bg-success/10', text: 'text-success' },
  vacant_clean:  { label: '空净',   bg: 'bg-success/10', text: 'text-success' },
  ...
  out_of_order:  { label: '维修',   bg: 'bg-secondary',    text: 'text-muted-foreground' },
  out_of_service:{ label: '停售',   bg: 'bg-secondary',    text: 'text-muted-foreground' },
  maintenance:   { label: '维护',   bg: 'bg-secondary',    text: 'text-muted-foreground' },
  retired:       { label: '停用',   bg: 'bg-secondary',    text: 'text-muted-foreground' },
}
...
const PRIORITY_MAP: Record<string, { label: string; cls: string }> = {
  ...
  normal: { label: '正常', cls: 'bg-secondary text-muted-foreground' },
  low:    { label: '低', cls: 'bg-secondary/50 text-muted-foreground' },
}
      ...
      {stats && (
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
          {[
            ...
            { label: '空净', value: stats.available, color: 'text-success' },
        ...
        <div className="flex bg-card rounded-xl p-1 shadow-sm border border-border">
          {[
            ...
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-1.5 px-4 py-2 text-sm rounded-lg font-medium transition-all ${tab === t.key ? 'bg-success/10 text-success shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
      ...
      {tab === 'rack' && (
        <>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-muted-foreground"><p className="text-sm">暂无客房</p></div>
          ) : (
            Object.entries(grouped).sort(([a],[b]) => a.localeCompare(b)).map(([floor, items]) => (
              <div key={floor}>
                <h4 className="text-sm font-medium text-muted-foreground mb-2">{floor} <span className="text-muted-foreground">({items.length})</span></h4>
                  ...
                  {items.sort((a, b) => a.roomNo.localeCompare(b.roomNo)).map(r => {
                    ...
                    return (
                      <div key={r._id} className="group relative">
                        <div className={`${st.bg} border-2 border-transparent rounded-xl p-3 text-center cursor-pointer transition-all hover:shadow-md hover:border-border`}
                          onClick={() => {
                          ...
                          <div className="text-[10px] text-muted-foreground">{ROOM_TYPES[r.roomType] || r.roomType}</div>
                          <div className={`text-[10px] font-medium ${st.text} mt-0.5`}>{st.label}</div>
                          {r.currentGuestName && <div className="text-[10px] text-muted-foreground truncate mt-0.5">{r.currentGuestName}</div>}
                          {r.currentStay?.expectedCheckOut && (
                            <div className="text-[9px] text-muted-foreground mt-0.5">退 {r.currentStay.expectedCheckOut.slice(5)}</div>
                          )}
                        </div>
          ...
          {[
            ...
            <div key={col.title} className={`bg-card rounded-2xl border-t-4 ${col.color} shadow-sm overflow-hidden`}>
              ...
              <div className="p-3 space-y-2 min-h-[120px]">
                {col.items.length === 0 ? (
                  <div className="text-center py-6 text-muted-foreground text-xs">暂无</div>
                ) : col.items.map(t => {
                  const pr = PRIORITY_MAP[t.priority] || PRIORITY_MAP.normal
                  return (
                    <div key={t._id} className="bg-secondary/50 rounded-xl p-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-foreground">{t.roomNo}</span>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${pr.cls}`}>{pr.label}</span>
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">{TASK_TYPES[t.taskType] || t.taskType}</div>
            ...
            {packages.length === 0 ? (
              ...
              <div key={pkg._id} className="bg-card rounded-2xl shadow-sm border border-border p-5">
                ...
                <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
                  <div>
                    <span className="text-lg font-bold text-foreground">¥{pkg.pricing.basePrice}</span>
                    {pkg.pricing.memberPrice > 0 && (
                      <span className="text-xs text-success ml-2">会员 ¥{pkg.pricing.memberPrice}</span>
                    )}
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-sm">
            ...
            <div className="px-6 py-5 space-y-4">
              <div><label className="block text-xs font-medium text-muted-foreground mb-1">客人姓名</label><input value={ciGuestName} onChange={e => setCiGuestName(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">人数</label><input type="number" value={ciGuestCount} onChange={e => setCiGuestCount(e.target.value)} min="1" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">预计退房</label><input type="date" value={ciExpectedCheckOut} onChange={e => setCiExpectedCheckOut(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-sm">
            ...
            <div className="px-6 py-5">
              <p className="text-sm text-muted-foreground">确认为 <strong>{actionRoom.currentGuestName || actionRoom.currentStay?.guestName || '客人'}</strong> 办理退房？</p>
              <p className="text-xs text-muted-foreground mt-2">退房后将自动创建清洁任务。</p>
            </div>
            <div className="flex gap-3 px-6 py-4 border-t border-border">
              <button onClick={() => { setActionRoom(null); setActionType(null) }} className="flex-1 px-4 py-2 border border-border text-muted-foreground text-sm rounded-lg hover:bg-secondary/50">取消</button>
      ...
      {showPkgForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">套餐名称</label><input value={pkgName} onChange={e => setPkgName(e.target.value)} placeholder="如：两球一晚" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">套餐代码</label><input value={pkgCode} onChange={e => setPkgCode(e.target.value)} placeholder="如：STAY_PLAY_1N" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
              </div>
              <div><label className="block text-xs font-medium text-muted-foreground mb-1">描述</label><input value={pkgDesc} onChange={e => setPkgDesc(e.target.value)} placeholder="如：1晚住宿 + 2轮果岭 + 双早" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">住宿晚数</label><input type="number" value={pkgNights} onChange={e => setPkgNights(e.target.value)} min="1" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">果岭轮数</label><input type="number" value={pkgRounds} onChange={e => setPkgRounds(e.target.value)} min="0" className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">散客价</label><input type="number" value={pkgBasePrice} onChange={e => setPkgBasePrice(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                <div><label className="block text-xs font-medium text-blue-600 mb-1">嘉宾价</label><input type="number" value={pkgPriceGuest} onChange={e => setPkgPriceGuest(e.target.value)} placeholder={pkgBasePrice} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
                <div><label className="block text-xs font-medium text-muted-foreground mb-1">周末加价</label><input type="number" value={pkgWeekendSurcharge} onChange={e => setPkgWeekendSurcharge(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></div>
              </div>
              <div className="grid grid-cols-4 gap-2">
                <div><label className="block text-xs font-medium text-success mb-1">普通会员</label><input type="number" value={pkgPriceMember1} onChange={e => setPkgPriceMember1(e.target.value)} placeholder={pkgMemberPrice || pkgBasePrice} className="w-full px-2 py-1.5 border border-border rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-ring" /></div>
