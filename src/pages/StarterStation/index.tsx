...
const STATUS_LABELS: Record<string, { label: string; color: string; dot: string }> = {
  ...
  front_9:    { label: '前9洞',   color: 'text-emerald-700 bg-emerald-50 border-emerald-200', dot: 'bg-emerald-400' },
  ...
  returned:   { label: '已回场',  color: 'text-gray-600 bg-gray-50 border-gray-200',      dot: 'bg-gray-400' },
  completed:  { label: '已完赛',  color: 'text-gray-500 bg-gray-50 border-gray-200',      dot: 'bg-gray-300' },
...
const DASHBOARD_CARDS = [
  ...
  { key: 'returned',      label: '已回场', icon: RotateCcw,     bg: 'bg-gray-50',    text: 'text-gray-600' },
  { key: 'completed',     label: '已完赛', icon: CheckCircle2,  bg: 'bg-gray-50',    text: 'text-gray-500' },
    ...
    <div className="min-h-screen bg-gray-50/80">
      ...
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-4">
          <button onClick={() => navigate('/home')} className="p-2 rounded-xl hover:bg-gray-100 transition-colors">
            <ArrowLeft size={20} className="text-gray-500" />
          </button>
          <div className="flex-1">
            <h1 className="text-lg font-bold text-gray-800">出发台管理</h1>
            <p className="text-xs text-gray-400">Starter Station</p>
          </div>
          ...
          <div className="flex items-center gap-1 bg-gray-50 rounded-xl p-1 border border-gray-200">
            ...
            <input type="date" value={date} onChange={e => setDate(e.target.value)}
              className="bg-transparent text-sm font-medium text-gray-800 px-1 py-1 w-[130px] outline-none" />
            <button onClick={() => setDate(d => shiftDate(d, 1))} className="p-1.5 rounded-lg hover:bg-white transition-colors">
              <ChevronRight size={16} className="text-gray-500" />
            </button>
            {date !== todayStr && (
              <button onClick={() => setDate(todayStr)} className="text-xs text-emerald-600 font-medium px-2 hover:bg-emerald-50 rounded-lg transition-colors">今天</button>
            )}
          </div>

          <button onClick={refresh} className="p-2 rounded-xl hover:bg-gray-100 transition-colors" title="刷新">
...
function QueuePanel({ date, refreshKey, onRefresh }: { date: string; refreshKey: number; onRefresh: () => void }) {
  ...
  if (list.length === 0) return <div className="text-center py-16 text-gray-300"><div className="text-3xl mb-2">🏌️</div><p className="text-sm">暂无待出发预订</p></div>

  return (
    <div className="space-y-3">
      <div className="text-xs text-gray-400">{list.length} 组待出发</div>

      {list.map(b => {
        const players = b.players || []
        const isExpanded = expandId === b._id
        return (
          <div key={b._id} className="bg-white rounded-xl border border-gray-150 overflow-hidden">
            <div className="px-4 py-3 flex items-center justify-between cursor-pointer hover:bg-gray-50/50 transition-colors"
              onClick={() => setExpandId(isExpanded ? null : b._id)}>
              <div className="flex items-center gap-3 min-w-0 flex-1">
                <div className="text-lg font-bold text-emerald-600 w-14 text-center flex-shrink-0">{b.teeTime}</div>
                <div className="min-w-0">
                  <div className="text-sm font-medium text-gray-800">{b.courseName || '未指定'} · {players.length}人</div>
                  <div className="text-xs text-gray-400 truncate">{players.map((p: any) => p.name).join('、')}</div>
                </div>
              </div>
            ...
            {isExpanded && (
              <div className="px-4 pb-3 pt-1 border-t border-gray-100 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                {b.caddyName && <div><span className="text-gray-400">预约球童</span><div className="font-medium text-gray-700 mt-0.5">{b.caddyName}</div></div>}
                {b.cartNo && <div><span className="text-gray-400">预约球车</span><div className="font-medium text-gray-700 mt-0.5">{b.cartNo}</div></div>}
                {b.note && <div className="col-span-2 sm:col-span-4"><span className="text-gray-400">备注</span><div className="font-medium text-gray-700 mt-0.5">{b.note}</div></div>}
                {players.length > 0 && (
                  <div className="col-span-2 sm:col-span-4">
                    <span className="text-gray-400">球员</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setDispatchForm(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs text-gray-500 mb-1 block">出发洞</label>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setDispatchForm(null)} className="flex-1 px-4 py-2.5 text-sm font-medium border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">取消</button>
              <button onClick={submitDispatch} className="flex-1 px-4 py-2.5 text-sm font-medium bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-colors">确认出发</button>
            </div>
          </div>
        </div>
      )}
...
function OnCoursePanel({ date, refreshKey, onRefresh }: { date: string; refreshKey: number; onRefresh: () => void }) {
  ...
  const nextActions = (status: string): { label: string; target: string; color: string }[] => {
    switch (status) {
      case 'dispatched': return [{ label: '开始前9洞', target: 'front_9', color: 'bg-emerald-600 hover:bg-emerald-700 text-white' }]
      case 'front_9':    return [
        { label: '转场', target: 'turning', color: 'bg-amber-500 hover:bg-amber-600 text-white' },
        { label: '回场(9洞)', target: 'returned', color: 'bg-gray-500 hover:bg-gray-600 text-white' },
      ]
      case 'turning':    return [{ label: '开始后9洞', target: 'back_9', color: 'bg-indigo-600 hover:bg-indigo-700 text-white' }]
      case 'back_9':     return [{ label: '回场', target: 'returned', color: 'bg-gray-600 hover:bg-gray-700 text-white' }]
      default: return []
    }
  }

  if (loading) return <div className="text-center py-12 text-gray-400 text-sm">加载中...</div>
  if (list.length === 0) return <div className="text-center py-16 text-gray-300"><div className="text-3xl mb-2">⛳</div><p className="text-sm">当前场上无球员</p></div>

  return (
    <div className="space-y-3">
      <div className="text-xs text-gray-400">{list.length} 组在场上</div>
      ...
      {list.map(b => {
        ...
        return (
          <div key={b._id} className="bg-white rounded-xl border border-gray-150 px-4 py-3">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-base font-bold text-emerald-600">{b.teeTime}</span>
                ...
                <div className="mt-1 text-sm text-gray-700">{b.courseName} · {players.length}人 · {players.map((p: any) => p.name).join('、')}</div>
...
function ReturnedPanel({ date, refreshKey, onRefresh }: { date: string; refreshKey: number; onRefresh: () => void }) {
  ...
  if (loading) return <div className="text-center py-12 text-gray-400 text-sm">加载中...</div>
  if (list.length === 0) return <div className="text-center py-16 text-gray-300"><div className="text-3xl mb-2">🏠</div><p className="text-sm">暂无回场记录</p></div>

  return (
    <div className="space-y-3">
      <div className="text-xs text-gray-400">{list.length} 组已回场/完赛/结账</div>
      ...
      {list.map(b => {
        ...
        return (
          <div key={b._id} className="bg-white rounded-xl border border-gray-150 px-4 py-3 flex items-center justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="text-sm font-bold text-gray-700">{b.teeTime}</span>
                <span className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full border ${sc.color}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${sc.dot}`} />
                  {sc.label}
                </span>
              </div>
              <div className="mt-0.5 text-xs text-gray-500">{b.courseName} · {players.map((p: any) => p.name).join('、')}</div>
              {totalFee > 0 && <div className="mt-0.5 text-xs font-medium text-gray-600">¥{totalFee.toLocaleString()}</div>}
            </div>
...
const BAG_STATUS: Record<string, { label: string; color: string }> = {
  stored:   { label: '在库', color: 'text-emerald-700 bg-emerald-50' },
  out:      { label: '已取出', color: 'text-amber-700 bg-amber-50' },
  returned: { label: '已归还', color: 'text-gray-500 bg-gray-50' },
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        {stats && (
          <div className="flex gap-3 text-xs">
            <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-lg font-medium">在库 {stats.stored}</span>
            <span className="px-2.5 py-1 bg-amber-50 text-amber-700 rounded-lg font-medium">已取出 {stats.out}</span>
            <span className="px-2.5 py-1 bg-gray-50 text-gray-500 rounded-lg font-medium">已归还 {stats.returned}</span>
          </div>
        )}
        <div className="relative flex-1 min-w-[160px] max-w-xs">
          ...
          <input value={keyword} onChange={e => setKeyword(e.target.value)}
            placeholder="搜索球员/编号"
            className="w-full pl-8 pr-3 py-1.5 text-sm bg-gray-50 border border-gray-200 rounded-lg outline-none focus:border-emerald-400" />
        </div>
        <button onClick={() => setFormOpen(true)}
          className="flex items-center gap-1 px-3 py-1.5 bg-emerald-600 text-white text-xs font-medium rounded-lg hover:bg-emerald-700 transition-colors ml-auto">
          <Plus size={14} /> 球包入库
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-300"><div className="text-3xl mb-2">🎒</div><p className="text-sm">暂无球包记录</p></div>
      ) : (
          ...
          {bags.map(b => {
            ...
            return (
              <div key={b._id} className="bg-white rounded-xl border border-gray-150 px-4 py-3 flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-gray-800">{b.playerName}</span>
                    <span className="text-xs font-mono text-gray-400">#{b.bagNo}</span>
                ...
                <div className="flex gap-1.5 flex-shrink-0">
                  ...
                  {b.status === 'out' && (
                    <button onClick={() => handleStatusChange(b._id, 'stored')}
                      className="px-2.5 py-1 text-xs font-medium bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors">归还入库</button>
                  )}
                </div>
              </div>
            )
      ...
      {formOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setFormOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-gray-800 mb-4">球包入库</h3>
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">球员姓名 *</label>
                  <input value={form.playerName} onChange={e => setForm(f => ({ ...f, playerName: e.target.value }))}
                    className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg outline-none focus:border-emerald-400" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">球包编号 *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))}
                    className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg outline-none focus:border-emerald-400" placeholder="例: A-12" />
                </div>
                <div>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setFormOpen(false)} className="flex-1 px-4 py-2.5 text-sm font-medium border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">取消</button>
              <button onClick={handleCreate} className="flex-1 px-4 py-2.5 text-sm font-medium bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-colors">确认入库</button>
            </div>
          </div>
        </div>
      )}
...
function TimelinePanel({ date, refreshKey }: { date: string; refreshKey: number }) {
  ...
  if (loading) return <div className="text-center py-12 text-gray-400 text-sm">加载中...</div>
  if (!data) return <div className="text-center py-16 text-gray-300"><p className="text-sm">无数据</p></div>
  ...
  const statusColor = (s: string) => {
    switch (s) {
      ...
      case 'front_9': return 'bg-emerald-500'
      case 'turning': return 'bg-amber-400'
      case 'back_9': return 'bg-indigo-500'
      case 'returned': case 'completed': return 'bg-gray-400'
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <button onClick={() => setViewMode('caddy')}
          className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${viewMode === 'caddy' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}>
          球童视图
        </button>
        <button onClick={() => setViewMode('cart')}
          className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${viewMode === 'cart' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}>
          球车视图
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-3 text-xs text-gray-500">
        {[
          ...
          { label: '前9洞', color: 'bg-emerald-500' },
          { label: '转场', color: 'bg-amber-400' },
          { label: '后9洞', color: 'bg-indigo-500' },
          { label: '回场/完赛', color: 'bg-gray-400' },
      ...
      {slotEntries.length === 0 ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="flex border-b border-gray-100">
            <div className="w-24 flex-shrink-0 px-3 py-2 text-xs font-medium text-gray-500 bg-gray-50 border-r border-gray-100">
            ...
            <div className="flex-1 relative h-8">
              {hours.map(h => {
                ...
                return (
                  <div key={h} className="absolute top-0 bottom-0 border-l border-gray-100" style={{ left: `${left}%` }}>
                    <span className="text-[10px] text-gray-400 ml-1">{String(h).padStart(2, '0')}:00</span>
                  </div>
                )
              })}
            </div>
          ...
          {slotEntries.map(([name, items]) => (
            <div key={name} className="flex border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
              <div className="w-24 flex-shrink-0 px-3 py-2.5 text-xs font-medium text-gray-700 border-r border-gray-100 truncate">{name}</div>
