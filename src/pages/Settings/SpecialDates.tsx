...
const DATE_TYPES = [
  ...
  { key: 'closed',      label: '封场日', color: 'bg-gray-200 text-gray-600 border-gray-300' },
]
...
const DATE_TYPE_MAP: Record<string, { label: string; bgClass: string; dotClass: string }> = {
  ...
  closed:     { label: '封', bgClass: 'bg-gray-100',  dotClass: 'bg-gray-500' },
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">日历管理</h2>
          <p className="text-sm text-gray-500 mt-0.5">标记节假日、会员日、赛事日和封场日，影响自动定价</p>
        </div>
        <button
      ...
      <div className="flex gap-4 flex-wrap">
        {DATE_TYPES.map(dt => (
          <div key={dt.key} className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-600">{dt.label}</span>
          </div>
        ))}
      ...
      {loading ? (
        ...
        <div className="border border-gray-200 rounded-xl overflow-hidden">
          ...
          <div className="grid grid-cols-7 bg-gray-50">
            {WEEKDAYS.map(w => (
              <div key={w} className="py-2 text-center text-xs font-semibold text-gray-500">{w}</div>
            ))}
          </div>
          {/* 日期 */}
            ...
            {days.map(({ date, day, inMonth }) => {
              ...
              return (
                <button
                  ...
                  disabled={!inMonth}
                  className={`relative h-20 p-1.5 border-b border-r border-gray-100 text-left transition-all
                    ${!inMonth ? 'opacity-30 cursor-default' : 'hover:bg-emerald-50 cursor-pointer'}
                    ${isToday ? 'ring-2 ring-emerald-400 ring-inset' : ''}
                    ${typeInfo ? typeInfo.bgClass : isWeekend ? 'bg-amber-50/50' : ''}
                  `}
                >
                  <div className="flex items-center gap-1">
                    <span className={`text-sm font-medium ${isToday ? 'text-emerald-600' : !inMonth ? 'text-gray-300' : isWeekend ? 'text-amber-600' : 'text-gray-700'}`}>
                      {day}
                    </span>
                    {typeInfo && (
                  ...
                  {sd && (
                    <div className="mt-0.5 text-[10px] text-gray-500 truncate">{sd.dateName}</div>
                  )}
                  {sd?.isClosed && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              ...
              <button onClick={() => setEditDate(null)} className="text-gray-400 hover:text-gray-700"><X size={20} /></button>
            </div>
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">日期类型</label>
                <div className="grid grid-cols-2 gap-2">
                  {DATE_TYPES.map(dt => (
                    <button
                      key={dt.key}
                      onClick={() => setEditData(p => ({ ...p, dateType: dt.key, pricingOverride: dt.key === 'closed' ? 'weekday' : dt.key }))}
                      className={`px-3 py-2 rounded-lg text-xs font-medium border transition-all ${editData.dateType === dt.key ? dt.color + ' ring-2 ring-offset-1' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                    >
                      {dt.label}
                    </button>
                  ))}
              ...
              <div>
                ...
                <input value={editData.dateName || ''} onChange={e => setEditData(p => ({ ...p, dateName: e.target.value }))} placeholder="如：劳动节" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" />
              </div>
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={editData.isClosed || false} onChange={e => setEditData(p => ({ ...p, isClosed: e.target.checked }))} className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-400" />
                  <span className="text-sm text-gray-700">封场</span>
                </label>
              </div>
              {editData.isClosed && (
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">封场原因</label>
                  <input value={editData.closedReason || ''} onChange={e => setEditData(p => ({ ...p, closedReason: e.target.value }))} placeholder="如：草坪维护" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" />
                </div>
              )}
            </div>
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100">
              ...
              <div className="flex gap-3">
                <button onClick={() => setEditDate(null)} className="px-4 py-2 text-sm text-gray-600">取消</button>
                <button onClick={handleSave} disabled={saving} className="px-5 py-2 bg-emerald-600 text-white text-sm rounded-lg hover:bg-emerald-700 disabled:opacity-50 font-medium">
                  {saving ? '保存中...' : '保存'}
                </button>
              </div>
            </div>
